 ...
 * @detail：非活体数据集
 */
class SpoofData :DataHelper<CameraFrame,CameraImage?> {
...
//            ImiUtil.irDataToRgb(irRgbBuffer,ir2Buffer,IMAGE_WIDTH, IMAGE_HEIGHT)
    ...
    private fun handlePath() {
        count++
        rgbPath = "${FileHelper.getInstance().faceTestFolderPath}spoof/${count}_color.jpg"
        rgbPath = when(count){
            ...
            in 100 until 1000-> rgbPath.replace("${count}","0${count}")
                ...
                "${FileHelper.getInstance().faceTestFolderPath}spoof/1000_color.jpg"
            }
        }
